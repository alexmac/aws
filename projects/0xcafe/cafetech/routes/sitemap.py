...
def _generate_sitemap(urls: List[URL]) -> str:
    urlset = etree.Element('urlset', attrib={}, nsmap={None: 'http://www.sitemaps.org/schemas/sitemap/0.9'})
...
async def get_sitemap(s: BlogServer, request):
    ...
    urls += [
        ...
        for project in valid_project_names
    ]
    
    return Response(body=_generate_sitemap(urls), headers={
